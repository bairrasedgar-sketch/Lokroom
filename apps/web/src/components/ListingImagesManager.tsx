...
function ListingPreviewCard({
  ...
  return (
    <div className="pointer-events-none flex justify-center">
      <div className="pointer-events-auto relative flex w-full max-w-md overflow-hidden rounded-2xl border bg-white shadow-xl">
        <button
          type="button"
          onClick={onClose}
          className="absolute right-2 top-2 z-10 h-7 w-7 rounded-full bg-black/60 text-xs text-white"
          aria-label="Fermer"
        >
          ✕
        </button>
        ...
        >
          <div className="relative h-28 w-32 flex-shrink-0 bg-gray-100">
            {cover ? (
              <Image
                src={cover}
                alt={listing.title}
      ...
      <main className="mx-auto flex min-h-[calc(100vh-80px)] max-w-6xl flex-col gap-4 px-4 pb-8 pt-4 lg:flex-row">
        ...
        <aside className="sticky top-24 hidden h-[600px] flex-1 lg:block">
          <div className="relative h-full w-full overflow-hidden rounded-3xl border bg-gray-100">
            ...
            {/* Pré-view d’annonce façon Airbnb (desktop) */}
            {selectedListing && (
      ...
      {showMobileMap && (
        <div className="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm sm:hidden">
          <div className="absolute inset-x-3 bottom-3 top-16 overflow-hidden rounded-3xl border bg-gray-100">
            ...
            {/* Pré-view d’annonce sur mobile aussi */}
            {selectedListing && (
