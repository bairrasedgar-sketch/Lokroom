  ...
  const fetchListingsInBounds = useCallback(
    async (bounds: { north: number; south: number; east: number; west: number }) => {
      ...
      try {
        ...
        // Add existing search params (sauf les filtres de localisation)
        // On exclut q, placeId, city, country, province car on veut TOUTES les annonces
        // dans la zone visible de la carte, peu importe la recherche initiale
        // On garde les autres filtres (type, prix, dates, etc.)
        const excludeKeys = ["page", "q", "placeId", "city", "country", "province", "neLat", "neLng", "swLat", "swLng"];
        ...
        {selectedMobileMarker && (
          ...
          >
            <div className="relative bg-white rounded-2xl shadow-xl overflow-hidden">
              {/* Bouton fermer - carré arrondi dans le coin */}
              <button
                type="button"
                onClick={(e) => {
                  e.preventDefault();
                ...
                className="absolute top-0 right-0 z-20 h-8 w-8 rounded-bl-2xl rounded-tr-2xl bg-gray-900/70 flex items-center justify-center"
              >
