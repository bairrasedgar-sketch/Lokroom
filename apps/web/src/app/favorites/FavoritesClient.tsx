...
export default function FavoritesClient() {
  const { wishlists: contextWishlists, wishlistsLoading: contextLoading } = useFavorites();

  // Use SWR for wishlists with cache
  ...
  const wishlistsData = wishlists || contextWishlists || [];
  ...
  const currentWishlist = wishlistsData.find(w => w.id === selectedWishlist);
  ...
  const totalFavorites = wishlistsData.reduce((acc, w) => acc + w._count.favorites, 0);
        ...
        <div>
          ...
          {totalFavorites > 0 && (
            <p className="mt-1 text-sm text-gray-500">
              {totalFavorites} annonce{totalFavorites !== 1 ? "s" : ""} dans {wishlistsData.length} liste{wishlistsData.length !== 1 ? "s" : ""}
            </p>
          )}
        </div>
